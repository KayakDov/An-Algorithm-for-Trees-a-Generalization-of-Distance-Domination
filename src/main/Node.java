...
public class Node {
    ...
    public class Neighbors {
        ...
        public Stream<Node> descendentGen(int gen) {
            ...
            return children().flatMap(child -> child.neighbors.descendentGen(gen
                    - 1));
        }
        ...
        public Stream<Node> descendents(int upToAndIncludingGen) {
            ...
            return Stream.concat(Stream.of(Node.this), children().flatMap(child -> child.neighbors.descendents(upToAndIncludingGen
                    - 1)));
        }
        ...
        public Stream<Node> siblings() {
            ...
            return parent.neighbors.children().filter(child -> child
                    != Node.this);
        }

        /**
         * All the nodes within the closed neighborhood of distance l of this
         * node.
         *
         * @param dist
         * @return
    ...
    public class Name {
        ...
        /**
         * adds 1 to the digit immediately preceding the last letter of the
         * proffered string.
         *
         * @param input the proffered string
         * @return a new string.
         */
        private String doubleLetter(String input) {
            ...
            return input.substring(0, i) + (numberFound + 1)
            ...
            IntStream.range(0, neighbors.numChildren())
                    .forEach(i -> {
                        ...
                        if (addOn == name.charAt(name.length() - 1)) {
                            neighbors.getChild(i).name.name = doubleLetter(name);
                        } else neighbors.getChild(i).name.name = name + addOn;
                    });
            neighbors.children().forEach(child -> child.name.setNames());
        }
        ...
        private String childNames() {
            return neighbors.children().map(child -> child.name != null ? child.name.name : "").reduce((s1, s2) -> s1
                    + ", " + s2).orElse("");
        }
    ...
    private int setComponentSizes() {
        if (isNearFailedNode) {
            neighbors.children.forEach(Node::setComponentSizes);
            return compSize = 0;
        }
     ...
     * @param l the distance of a node from a failed node to be considered near
     ...
     */
    public int minFailureSetSize() {
        return (isSelectedForFailureSet ? 1 : 0) + 
                neighbors.children()
                .mapToInt(node -> node.minFailureSetSize())
                .sum();
    }
    ...
    public String toString(String indent) {
        String local = indent + name + ": "
                + (isSelectedForFailureSet ? "O" : isNearFailedNode ? "X" : "")
                + "\n";
        if (neighbors.hasChildren()) {
            return local
                    + neighbors.children().map(child -> child.toString(indent
                    + "\t")).reduce((s1, s2) -> s1
                    + s2).get();
        }
        return local;
